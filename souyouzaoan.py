...
def get_tianhang():
    try:
        ...
        response = get(url, headers=headers).json()
        if response["code"] == 200:
            one = response["newslist"][0]["content"]
        else:
            one = ""
    except KeyError:
        one = ""
...
def get_tianhang():
    try:
        ...
        response = get(url, headers=headers).json()
        if response["code"] == 200:
            joke = response["newslist"][0]["content"]
        else:
            joke = ""
    except KeyError:
       joke = ""
...
def get_tianhang():
    try:
        ...
        response = get(url, headers=headers).json()
        if response["code"] == 200:
           dujitang = response["newslist"][0]["content"]
        else:
            dujitang = ""
    except KeyError:
        dujitang = ""
...
def get_tianhang():
    try:
        ...
        response = get(url, headers=headers).json()
        if response["code"] == 200:
            zaoan = response["newslist"][0]["content"]
        else:
            zaoan = ""
    except KeyError:
       zaoan = ""
...
def get_tianhang():
    try:
        ...
        response = get(url, headers=headers).json()
        if response["code"] == 200:
            wanan = response["newslist"][0]["content"]
        else:
            wanan = ""
    except KeyError:
        wanan = ""
...
if __name__ == "__main__":
    ...
    chp,zaoan,wanan,dujitang,one,joke = get_tianhang()
    # 公众号推送消息
    for user in users:
